...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'stock_data_app_2024'
...
def update_stock_data():
    ...
    while True:
        try:
            print(f"[{datetime.now()}] 正在更新股票数据...")
            new_data = fetcher.get_multiple_stocks_data()
            if new_data:
                current_stock_data = new_data
                last_update_time = datetime.now()
                print(f"[{datetime.now()}] 股票数据更新完成，共 {len(new_data)} 只股票")
            else:
                print(f"[{datetime.now()}] 股票数据更新失败")
        except Exception as e:
            print(f"[{datetime.now()}] 更新股票数据时出错: {e}")
...
<body>
    <div class="container">
        ...
        <div id="stockContainer" class="stock-grid" style="display: none;">
        </div>
        
        <div class="footer">
            <p>数据来源于公开接口，仅供参考，不构成投资建议 | 更新时间：<span id="updateTime"></span></p>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadStockData() {
            try {
                ...
                if (result.success) {
                    ...
                    updateStatus(result.last_update, result.count);
                } else {
                    showError('获取数据失败');
                }
            } catch (error) {
                ...
                html += `
                    <div class="stock-card ${changeClass}">
                        ...
                        <div class="stock-change">
                            ${changeSymbol}${stock.change.toFixed(2)} (${changeSymbol}${stock.change_pct.toFixed(2)}%)
                        </div>
        ...
        // 更新状态信息
        function updateStatus(lastUpdate, count) {
            document.getElementById('lastUpdate').textContent = 
                lastUpdate ? `最后更新: ${lastUpdate}` : '未更新';
        ...
        function toggleAutoRefresh() {
            if (isAutoRefresh) {
                ...
                isAutoRefresh = false;
                document.querySelector('button[onclick="toggleAutoRefresh()"]').textContent = '⏰ 自动刷新';
            } else {
                autoRefreshInterval = setInterval(loadStockData, 60000); // 每分钟刷新
                isAutoRefresh = true;
...
def main():
    ...
    global current_stock_data, last_update_time
    print("正在初始化股票数据...")
    initial_data = fetcher.get_multiple_stocks_data()
    if initial_data:
        current_stock_data = initial_data
        last_update_time = datetime.now()
        print(f"初始化完成，获取到 {len(initial_data)} 只股票数据")
    else:
        print("初始化失败，将使用空数据启动")
